...
import {
  shineCard,
  getCardWithParentHtml,
  createCardWithParentDom,
  random,
  removeShineCard,
} from "./handleElements";
...
const transform = (targets, duration) => {
  if (!targets || targets.length === 0) {
    console.error("Error: targets 未正确初始化或为空！");
    return;
  }

  if (targets.length < paramsFields.threeDCards.length) {
    console.error(
      `Error: targets 长度 (${targets.length}) 小于 threeDCards 长度 (${paramsFields.threeDCards.length})`
    );
    return;
  }
  ...
  for (var i = 0; i < paramsFields.threeDCards.length; i++) {
    ...
    if (!target || !target.position || !target.rotation) {
      console.error(`Error: targets[${i}] 属性不完整！`, target);
      continue;
    }
...
const enterAnimate = (resetPrizeStatus = true) => {
  ...
  const RADIUS = 800; // 增大螺旋的半径，使卡片之间的间距更大
  const HEIGHT = 1200; // 增大螺旋的高度，拉开高度差

  const TOTAL_POINTS = paramsFields.threeDCards.length;
  //向下取整
  const SPIRAL_TURNS = Math.floor(TOTAL_POINTS / 40); // 增加螺旋的圈数
  console.log(SPIRAL_TURNS, "SPIRAL_TURNS");
  // const SPIRAL_TURNS = TOTAL_POINTS/80; // 增加螺旋的圈数

  // 计算螺旋的高度范围的中心
const centerY = HEIGHT / 2; // 将整个螺旋群体的高度范围平移到中间位置
console.log(TOTAL_POINTS, 'TOTAL_POINTS')
for (let i = 0; i < TOTAL_POINTS; i++) {
  const angle = (i / TOTAL_POINTS) * Math.PI * SPIRAL_TURNS; // 每点的角度，增加圈数
  const y = (i / TOTAL_POINTS) * HEIGHT - centerY; // 高度差，平移到中心
  ...
  // 手动设置卡片正对屏幕
  object.rotation.set(0, 0, 0); // 先重置 rotation
  object.rotation.y = Math.atan2(-z, -x) + Math.PI / 2; // 根据位置调整旋转角度
  ...
  // 动画效果：螺旋旋转
  gsap.to(targets.spiral, {
    duration: 12, // 增加旋转时间，以适应更多的圈数
    rotationY: "+=360", // 整体旋转
    repeat: -1,
    ease: "linear",
  });
...
const findCurrentLotteryGroup = () => {
  ...
  const userGroup = basicData.groupList.find((group) =>
    group.options.includes(currentPrize.type)
  );
...
const initParamsFieldsData = (userGroup, isSwitchAnimation) => {
  ...
  const member = JSON.parse(
    JSON.stringify(basicData.memberListData[userGroup.group_identity] || [])
  );
  if (member.length <= 0 && basicData.prizes.length > 0) {
    ElMessage({
      type: "warning",
      message: `未找到${
        basicData.currentPrize ? basicData.currentPrize.name : ""
      }的抽奖人员名单,请检查人员名单和奖项关联设置情况`,
    });
...
const rotateBall = (isAutoRate = false) => {
  const confirmRotateTime = !isAutoRate
    ? basicData.ballConfig.rotateTime
    : basicData.ballConfig.autoRotateTime;
...
const findAndMerge = (arr, input, current) => {
  try {
    ...
    const rows = arr.filter((row) => {
      ...
      const secretUser = basicData.secretUsers.find(
        (user) => user.option_identity === row[0] + ""
      );

      if (secretUser) {
        const [id, department, name, drawCount] =
          secretUser.option_value.split(",");
...
const cheatingUser = () => {
  try {
    ...
    if (secretPrizesGroupList && secretPrizesGroupList.length) {
      const secretGroup = secretPrizesGroupList.find(
        (group) => group.group_identity === basicData.currentPrize.type
      );
      if (secretGroup) {
        basicData.currentLuckys = findAndMerge(
          paramsFields.member,
          secretGroup.options,
...
const lottery = () => {
  ...
  rotateBall().then(() => {
    ...
    let leftPrizeCount =
      basicData.currentPrize.count - (luckyData ? luckyData.length : 0);
    ...
    for (let i = 0; i < perCount; i++) {
      ...
      if (getLuckyObj && getLuckyObj[0]) {
        let bindUserDom = document.querySelector(
          `.bind-user-${getLuckyObj[0]}`
        );
        if (bindUserDom) {
...
const saveData = () => {
  ...
  if (basicData.currentLuckys && basicData.currentLuckys.length > 0) {
    ...
    return myApi.saveOneRoundLuckyData(
      type,
      JSON.stringify(basicData.luckyUsers[type])
    );
  }
  return Promise.resolve();
};
    ...
    new TWEEN.Tween(this)
      ...
      .onComplete(() => {
        paramsFields.selectedCardIndex.forEach((index) => {
          ...
          // debugger
          if (
            object.element.classList.value.includes("prize") &&
            model === "lottery"
          ) {
        ...
        paramsFields.threeDCards = paramsFields.threeDCards.filter(
          (item) => item
        );
        // if (model === "lottery") {
        //   currentLuckys = []
...
const lotteryActiveFn = async () => {
  ...
  if (!basicData.isNextPrize) {
    ...
    if (basicData.currentLotteryGroup) {
      ...
      if (
        userGroup.group_identity !==
        basicData.currentLotteryGroup.group_identity
      ) {
        const loading = ElLoading.service({
          lock: true,
          text: "人员名单有变动,正在切换抽奖人员",
    ...
    lottery("lottery");
    toast.info(
      `正在抽取[${basicData.currentPrize.name}],屏住呼吸，幸运或许就是你！✨`,
      {
        timeout: 2000,
      }
    );
  ...
  resetCard(500, "lottery").then(async (res) => {
    ...
    lottery("lottery");
    toast.info(
      `正在抽取[${basicData.currentPrize.title}],屏住呼吸，幸运或许就是你！✨`,
      {
        timeout: 2000,
      }
    );
...
const beginLottery = () => {
  ...
  if (leftCount < perCount) {
    toast.error(
      `剩余${leftCount}人，而每轮要抽${perCount}人；所有参与抽奖人员不足，不能进行下去；可去“奖项设置”调整每轮抽奖个数或调整所有参与抽奖人员！`,
      {
        timeout: 5000,
      }
    );
...
const resetBtnClick = async () => {
  ...
  basicData.memberListData = JSON.parse(
    JSON.stringify(basicData.originMemberListData)
  );
...
const reLottery = () => {
  ...
  paramsFields.member = JSON.parse(
    JSON.stringify(paramsFields.member.concat(basicData.currentLuckys))
  );
  // 过滤
...
const resetCurrentPrizeBtnClick = async () => {
  ...
  if (basicData.luckyUsers[type]) {
    ...
    const userGroup = findCurrentLotteryGroup();
    let lotteredUserMap = getLotteredUserMap();
    let currentGroupData = JSON.parse(
      JSON.stringify(basicData.originMemberListData[userGroup.group_identity])
    );
    // 过滤已中奖的名单
    currentGroupData = currentGroupData.filter(
      (user) => !lotteredUserMap[user[0]]
    );
...
const adjustShineUser = (switchTime) => {
  ...
  if (
    paramsFields.threeDCards &&
    paramsFields.threeDCards.length <= 0 &&
    !basicData.isEnterLottery
  ) {
    initCards();
  }

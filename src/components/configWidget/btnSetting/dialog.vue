<template>
  <el-dialog
    ...
    width="70%">
    <template #header>
      <slot name="title"><span class="title-text">{{ textMappingConfig.btnSetting.chineseText + ' ' + textMappingConfig.btnSetting.otherLanguagesText }}</span></slot>
...
import btnSetting from './index.vue'
const basicData = lotteryDataStore();
const dialogTableVisible = ref(false);
const dialogKeyStr = 'btnSetting';
...
const confirm = async () => {
  window.operationLogTable.add({
    ...
    type: 'setting',
    value: textMappingConfig.value.btnSetting.chineseText + ' ' + textMappingConfig.value.btnSetting.otherLanguagesText
  })
  const isPass = await btnSettingRef.value.validateForm();
  if (!isPass) {
    return
  }
  // 删除不必存的字段
  const btnAboutData = JSON.parse(JSON.stringify(btnSettingRef.value['formLabelAlign']));
  const btnAboutDataPass = await handleVerifyConfig('btnSettings', btnAboutData);
   // 检查所有状态
  const status = checkAllPassStatus(
    btnAboutDataPass.status
  );
